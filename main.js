...
var SyncEngine = class {
  ...
  resolveStateActions(localState, remoteState, direction) {
    ...
    for (const path2 of allPaths) {
      const lEntry = localFiles[path2] || {};
      const rEntry = remoteFiles[path2] || {};
      const lv = lEntry.v || 0;
      const rv = rEntry.v || 0;
      const lExists = path2 in localFiles ? (_a = lEntry.exists) != null ? _a : true : false;
      const rExists = path2 in remoteFiles ? (_b = rEntry.exists) != null ? _b : true : false;
      if (direction === "upload") {
        if (lv > rv) {
          if (lExists && !hashMatch(lEntry, rEntry)) {
            actions.push({ path: path2, action: "upload" });
          } else if (rExists && !lExists) {
            actions.push({ path: path2, action: "delete" });
          }
        } else if (rv > lv && !(path2 in localFiles) && rExists) {
          actions.push({ path: path2, action: "delete" });
        }
      } else if (direction === "download") {
        if (rv > lv) {
          if (rExists && !hashMatch(lEntry, rEntry)) {
            actions.push({ path: path2, action: "download" });
          } else if (lExists && !rExists) {
            actions.push({ path: path2, action: "delete_local" });
          }
        } else if (lv > rv && !(path2 in remoteFiles) && lExists) {
          actions.push({ path: path2, action: "delete_local" });
        }
      } else {
        if (lv > rv) {
  ...
  mergeStates(localState, remoteState) {
    ...
    for (const path2 of allPaths) {
      const lEntry = (localState.files || {})[path2] || {};
      const rEntry = (remoteState.files || {})[path2] || {};
      const lv = lEntry.v || 0;
      const rv = rEntry.v || 0;
      merged.files[path2] = lv >= rv ? lEntry : rEntry;
    }
    return merged;
  }
  // ─── 执行单个操作 ───────────────────────────────────────────────────
  ...
  async dropboxListFolder(path2, recursive = true) {
...
var DropboxSyncPlugin = class extends import_obsidian4.Plugin {
  ...
  initSyncEngine() {
    ...
    const pluginDir = basePath ? path.join(basePath, this.app.vault.configDir || ".obsidian", "plugins", this.manifest.id) : __dirname;
